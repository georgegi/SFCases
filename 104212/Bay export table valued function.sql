...
-- select * from x_PORT.Enrich2FOCUSExport()
...
left join PrgInvolvement inv on s.ID = inv.StudentID
	...
	-- new (only if there is a finalized IEP) and only the most recent involvement
	and inv.ID = (
		...
		join PrgInvolvement inv2 on i.InvolvementID = inv2.ID and inv2.ProgramID = 'F98A8EF2-98E2-4CAC-95AF-D7D89EF7F80C'
		where i.StudentID = s.ID
		order by inv2.EndDate
		)
where 
...
join PrgStatus e on i.StatusID = e.ID and e.IsExit = 1
where t.IEPYN = '1'
--and e.IsExit = 1
--and i.EndDate <= getdate()


--select s.Name, s.StateCode, s.IsExit, invs.* 
--from PrgInvolvementStatus invs
--join PrgStatus s on invs.StatusID = s.ID
--where invs.InvolvementID = '57C57325-5E44-470F-90C2-34C38EF51AF1'
--and invs.StartDate = (
--	select MAX(im.StartDate)
--	from PrgInvolvementStatus im
--	where invs.InvolvementID = im.InvolvementID)
---- 
...
--select t.*
-- select s.FirstName, s.LastName, s.*
--from @t t
